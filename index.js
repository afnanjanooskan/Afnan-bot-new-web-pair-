...
async function startSession(userId, pairingPhone = null) {
  ...
  // Use pairing code mode if phone number provided and not already registered
  const usePairingCode = !!pairingPhone && !state.creds.registered;
  ...
  // ── Request pairing code after socket opens ───────────────────────────────
  if (usePairingCode) {
    ...
    setTimeout(async () => {
      try {
        ...
        emitToUser(userId, 'logs', `💡 Try QR method or check the phone number format`);
      }
    }, 2000);
  }
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    // QR generated (used when NOT in pairing code mode)
    ...
    if (connection === 'open') {
      ...
      // Register this phone as owner if first pairing
      let isOwner = false;
    ...
    if (connection === 'close') {
      ...
      if (loggedOut) {
        emitToUser(userId, 'logs', '🚪 Logged out — please reconnect');
        await destroySession(userId, true);
      } else {
