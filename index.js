...
async function startSession(userId, pairingPhone, reconnectAttempt) {
  ...
  sock.ev.on('connection.update', async function(update) {
    ...
    if (connection === 'close') {
      ...
      // 515 restartRequired — WhatsApp sends this right after QR scan to signal
      // "reconnect now with the credentials you just saved."  Must NOT delete
      // the session directory — creds.json was just written. Reconnect at once.
      if (restartNeeded) {
        emitToUser(userId, 'logs', 'QR scanned ✅ — completing connection…');
        setStatus(userId, 'connecting');
        startSession(userId, null, 0);
        return;
      }
